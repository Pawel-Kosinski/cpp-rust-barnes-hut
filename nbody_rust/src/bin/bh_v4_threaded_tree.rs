...
fn calculateForces(pIdx: usize, arena: &[Node], particles: &[Particle]) -> (f32, f32)
{
    let target = &particles[pIdx];
    let mut acc_x = 0.0;
    let mut acc_y = 0.0;
    ...
    while currNodeIdx != usize::MAX
    {
        ...
        if node.children[0] == usize::MAX {
            ...
            while source_idx != usize::MAX {
                if source_idx != pIdx {
                    let source = &particles[source_idx];
                    let dx = source.pos_x - target.pos_x;
                    let dy = source.pos_y - target.pos_y;
        ...
        let dx = node.center_of_mass_x - target.pos_x;
        let dy = node.center_of_mass_y - target.pos_y;
        let dist_sq = dx * dx + dy * dy;
        let contains_target = (target.pos_x - node.bounds_x).abs() <= node.half_size
            && (target.pos_y - node.bounds_y).abs() <= node.half_size;
...
fn mainMain(options: &benchmark_options::BenchmarkOptions)
{
    ...
    for frame in 0..options.total_frames() {
        ...
        for i in 0..particles.len()
        {
            let force = calculateForces(i, &arena, &particles);
            particles[i].acc_x += force.0;
            particles[i].acc_y += force.1;
        }
